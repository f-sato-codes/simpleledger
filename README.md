...
実装前に、必要な画面を洗い出し、  
各画面の役割を整理した上で設計・実装を行いました。

| 画面ID | 画面名 | URL | 主な役割 |
|------|------|------|------|
| G01 | ログイン | `/login` | ユーザー認証 |
| G02 | 収支一覧 | `/transactions` | 月別収支一覧・月次サマリー表示 |
| G03 | 収支登録 | `/transactions/create` | 収支データの新規登録 |
| G04 | 収支編集 | `/transactions/{id}/edit` | 登録済み収支データの編集 |
| G05 | カテゴリ管理 | `/categories` | カテゴリの管理（CRUD） |

CRUD の流れが把握しやすいよう、  
収支に関する処理は `TransactionController` に集約し、  
カテゴリ管理は `CategoryController` で管理しています。
